...
int rectgetnum(struct nd_image *imgorig, struct nd_vector3 p[4],
...
printf("%lf\n", (te.tv_sec - ts.tv_sec) + (te.tv_nsec - ts.tv_nsec) * 1e-9);
		...
		for (rn = 0; rn < rc; ++rn) {
			...
			addtop = (double) abs(r[rn].y0 - r[rn].y1)
				* 0.2;
			addbot = (double) abs(r[rn].y0 - r[rn].y1)
				* 0.4;

			rectgetorigpos(r + rn, &perspmat,
				addtop, addbot,
				(double) img->w / IMGWIDTH, p);

			winw = (r[rn].x1 - r[rn].x0)
				* (double) img->w / IMGWIDTH;
			winh = (r[rn].y1 - r[rn].y0 + addtop + addbot)
				* (double) img->w / IMGWIDTH;

			if (winw >= img->w && winh >= img->h)
				continue;
		
			if (rectgetnum(img, p, winw, winh, &imgnum)
				< 0)
				return (-1);
